...
export function PortfolioSummary({ portfolio }: PortfolioSummaryProps) {
  const { summary, dataMode } = portfolio;

  const cards = [
    {
      label: "Total Value",
      value: summary.totalValue,
      sub: `${summary.dailyChange} today`,
      icon: Wallet,
    },
    {
      label: "24h Change",
      value: summary.dailyPercent,
      sub: "Portfolio growth",
      icon: BarChart3,
    },
    {
      label: "Route Readiness",
      value: summary.routeReadiness,
        ...
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className={cn(
                "rounded-3xl border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl",
                portfolio.isLoading && "animate-pulse",
              )}
            >
